...
class _RecordsScreenState extends State<RecordsScreen> {
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: [
      Text('Rekod & Statistik', style: Theme.of(context).textTheme.headlineMedium),
      const SizedBox(height: 8),
      const Text('Jejaki kemajuan dan semak semula jawapan kuiz anda.'),
      const SizedBox(height: 20),
      RecordsBuilder(repository: widget.repository, builder: (context, repository) {
        ...
        if (records.isEmpty) {
          return const Card(child: Padding(
            ...
            child: Column(children: [
              ...
              Text('Selesaikan kuiz di tab Kuiz. Keputusan anda akan disimpan di sini.'),
            ]),
          ));
        }
        ...
        final chapters = records.where((r) => r.form == _form)
          .map((r) => r.chapter).toSet().toList()..sort();
        final groups = <String, List<QuizRecord>>{};
        for (final record in filtered) {
          final label = _form == null ? 'Tingkatan ${record.form}' :
        ...
        return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            for (final form in <int?>[null, 1, 2, 3, 4, 5])
              ChoiceChip(
                ...
                onSelected: (_) => setState(() {
                  _form = form; _chapter = null; _limit = 20;
                }),
              ),
          ]),
          if (_form != null) ...[
            ...
            Wrap(spacing: 8, runSpacing: 8, children: [
              for (final chapter in <int?>[null, ...chapters])
                ChoiceChip(
                  label: Text(chapter == null ? 'Semua bab' : 'Bab $chapter'),
                  selected: _chapter == chapter,
                  onSelected: (_) => setState(() { _chapter = chapter; _limit = 20; }),
                ),
            ]),
          ],
          const SizedBox(height: 20),
          if (filtered.isEmpty)
            ...
            const SizedBox(height: 20),
            Text('Prestasi mengikut ${_form == null ? 'Tingkatan' : 'Bab'}',
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.stretch,
                  children: [
                    ...
                    LinearProgressIndicator(
                      value: RecordStatistics(groups[label]!).average / 100,
                      semanticsLabel: 'Purata $label',
                    ),
                    const SizedBox(height: 8),
                    Text('${groups[label]!.length} percubaan • Purata '
            ...
            const SizedBox(height: 20),
            Text('Sejarah Percubaan', style: Theme.of(context).textTheme.titleLarge),
            const Text('Terbaharu dahulu • Ketik rekod untuk semakan jawapan.'),
            for (final record in filtered.take(_limit)) RecordTile(record: record),
            if (filtered.length > _limit)
              TextButton(onPressed: () => setState(() => _limit += 20),
                child: const Text('Lihat lebih banyak')),
          ],
          const SizedBox(height: 16),
          const Text('Rekod disimpan pada peranti atau pelayar ini sahaja. '
...
class StatisticsSummary extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Wrap(spacing: 12, runSpacing: 12,
    children: [
      _metric(context, 'Percubaan', '${statistics.count}'),
      _metric(context, 'Skor terbaik', '${statistics.best.toStringAsFixed(1)}%'),
      _metric(context, 'Purata skor', '${statistics.average.toStringAsFixed(1)}%'),
