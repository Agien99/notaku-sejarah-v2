...
void main() {
  ...
  test(
    'real bank supports 15 unique questions without mutating source data',
    () async {
      final bank = await QuizRepository().getQuestions(form: 1, chapter: 1);
      final originalIds = bank.map((q) => q.id).toList();
      final originalOptions = bank
          .map((q) => q.options.map((o) => o.id).toList())
          .toList();
      ...
      expect(
        bank.map((q) => q.options.map((o) => o.id).toList()),
        originalOptions,
      );
      expect(
        session.questions.map((q) => q.id),
        isNot(originalIds.take(15).toList()),
      );
      expect(
      ...
      for (final question in session.questions) {
        expect(
          ...
          bank.singleWhere((q) => q.id == question.id).correctOption.text,
        );
      }
      final next = QuizSession(bank, random: Random(2));
      expect(
        next.questions.map((q) => q.id),
        isNot(session.questions.map((q) => q.id).toList()),
      );
    },
  );
  ...
  test('scores stable IDs, allows edits, locks submitted answers and resets retries', () async {
    ...
    session.answer(
      0,
      first.options.firstWhere((o) => o.id != first.correctOptionId).id,
    );
