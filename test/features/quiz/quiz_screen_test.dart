...
void main() {
  Future<void> openAttempt(WidgetTester tester, {QuizRepository? repository}) async {
    final loadedRepository = repository ?? _LoadedRepository(
      (await tester.runAsync(() => QuizRepository().getQuestions(form: 1, chapter: 1)))!,
    );
  ...
  testWidgets('selects form and exposes unavailable chapters without starting them', (tester) async {
    await tester.pumpWidget(MaterialApp(
      theme: AppTheme.light,
    ...
    final unavailable = tester.widget<ListTile>(find.ancestor(
  ...
  testWidgets('completes 15 questions, reviews answers and starts a fresh retry', (tester) async {
    tester.view.devicePixelRatio = 1;
    tester.view.physicalSize = const Size(390, 844);
    addTearDown(tester.view.reset);
    await openAttempt(tester);
    expect(find.text('0/15 dijawab'), findsOneWidget);
    expect(tester.widget<FilledButton>(find.widgetWithText(FilledButton, 'Hantar jawapan')).onPressed, isNull);
    for (var i = 0; i < 15; i++) {
    ...
    expect(tester.widget<Semantics>(find.ancestor(
      of: find.byKey(const ValueKey('answer-0')),
      matching: find.byType(Semantics),
    ).first).properties.selected, isTrue);
  ...
  testWidgets('system back asks before discarding an active attempt', (tester) async {
    await openAttempt(tester);
  ...
  testWidgets('shows a retryable load error and rejects undersized banks', (tester) async {
    final repository = _UnavailableRepository();
    await openAttempt(tester, repository: repository);
    expect(find.text('Soalan tidak dapat dimuatkan. Sila cuba lagi.'), findsOneWidget);
  ...
  for (final size in [const Size(320, 568), const Size(844, 390), const Size(1024, 768)]) {
    testWidgets('quiz scrolls without overflow at $size with larger text', (tester) async {
      tester.view.devicePixelRatio = 1;
...
class _UnavailableRepository extends QuizRepository {
  ...
  @override
  Future<List<QuizQuestion>> getQuestions({required int form, required int chapter}) async {
...
class _LoadedRepository extends QuizRepository {
  ...
  @override
  Future<List<QuizQuestion>> getQuestions({required int form, required int chapter}) async => questions;
}
