...
QuizRecord sampleRecord(String id, {int form = 1, int chapter = 1,
  int score = 1, int total = 2, int day = 1}) => QuizRecord(
  id: id, form: form, chapter: chapter, title: 'Mengenali Sejarah',
  startedAt: DateTime.utc(2026, 9, day, 10),
  completedAt: DateTime.utc(2026, 9, day, 10, 2),
  answers: List.generate(total, (index) => RecordedAnswer(
    questionId: 'q$index', prompt: 'Soalan $index',
    selectedAnswer: index < score ? 'Betul' : 'Salah',
    correctAnswer: 'Betul', isCorrect: index < score,
    explanation: 'Penerangan $index',
  )),
);

void main() {
  test('disk reopen preserves answer snapshots; repeated save is idempotent', () async {
    final directory = await Directory.systemTemp.createTemp('notaku-records-');
  ...
  test('database open failure is visible and retry can recover', () async {
    ...
    final repository = RecordsRepository(openDatabase: () async {
    ...
    try {
      ...
      await expectLater(repository.save(sampleRecord('retry')), throwsStateError);
      fail = false;
