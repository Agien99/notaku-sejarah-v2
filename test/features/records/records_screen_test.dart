...
void main() {
  for (final size in [const Size(320, 568), const Size(844, 390), const Size(1024, 768)]) {
    testWidgets('records filter and detail work at $size with large text', (tester) async {
      ...
      await tester.pumpWidget(MaterialApp(home: Scaffold(
        body: SingleChildScrollView(child: RecordsScreen(repository: repository)),
      )));
  ...
  testWidgets('empty records show guidance without invented statistics', (tester) async {
    await tester.pumpWidget(const MaterialApp(home: Scaffold(
      body: SingleChildScrollView(child: RecordsScreen()),
    )));
    await tester.pumpAndSettle();
  ...
  testWidgets('home updates when a completed record is saved', (tester) async {
    ...
    await tester.pumpAndSettle();
    expect(find.byKey(const ValueKey('latest-performance-empty')), findsNothing);
